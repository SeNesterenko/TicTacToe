...
namespace Services
{
    public static class GameExtensions
    {
        public static int GetLongestChain(this Dictionary<Vector2Int, EcsEntity> cells, Vector2Int position)
        {
            ...
            if (!startEntity.Has<TakenSignComponent>())
            {
                return 0;
            }
        
            var startType = startEntity.Ref<TakenSignComponent>().Unref().Sign;
            ...
            Vector2Int direction, Vector2Int position)
        {
            ...
            {
                if (!entity.Has<TakenSignComponent>())
                {
                    break;
                }

                var type = entity.Ref<TakenSignComponent>().Unref().Sign;
                if (type != startType)
                {
                    break;
                }
